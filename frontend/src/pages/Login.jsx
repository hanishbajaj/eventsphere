...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-base)', padding: '40px 24px', position: 'relative', overflow: 'hidden' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ width: '100%', maxWidth: 440, position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <Link to="/" className="nav-logo" style={{ fontSize: '1.8rem' }}>Event<span>Sphere</span></Link>
          <p style={{ color: 'var(--text-secondary)', marginTop: 8, fontSize: '0.9rem' }}>Welcome back</p>
        </div>
        <div className="card" style={{ padding: '40px', borderColor: 'var(--border-gold)' }}>
          <form onSubmit={handleSubmit} noValidate>
            ...
            <motion.button type="submit" className="btn btn-gold"
              ...
              {loading ? <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div className="spinner" style={{ width: 18, height: 18, borderWidth: 2 }} />Signing in...</span> : 'Sign In →'}
            </motion.button>
          </form>
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ textAlign: 'center', fontSize: '0.72rem', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>Demo Accounts</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
            {demoAccounts.map(acc => (
              <motion.button key={acc.role} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                onClick={() => { setForm({ email: acc.email, password: acc.password }); setErrors({}); setTouched({}); }}
                style={{ padding: '8px 12px', borderRadius: 'var(--radius-sm)', background: `${acc.color}12`, border: `1px solid ${acc.color}30`, color: acc.color, fontSize: '0.78rem', fontWeight: 500, textTransform: 'capitalize', cursor: 'pointer' }}>
                {acc.role}
              </motion.button>
            ))}
          </div>
          <p style={{ textAlign: 'center', fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 8 }}>Click to auto-fill credentials</p>
        </div>
      </motion.div>
    </div>
  );
